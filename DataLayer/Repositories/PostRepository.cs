...
namespace DataLayer
    {
    public class PostRepository : IDB<Post, int>
        {
        private GloNetDbContext _context;

        public PostRepository(GloNetDbContext context)
            {
        ...
        public Post Read(int key, bool noTracking = false, bool useNavigationProperties = false)
            ...
            try
                ...
                if (useNavigationProperties)
                    {
                    posts = posts.Include(p => p.Photo).Include(p => p.User).Include(p => p.Reactions).Include(p => p.Comments);
                    }
