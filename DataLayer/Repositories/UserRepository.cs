...
namespace DataLayer
    {
    public class UserRepository : IDB<User, int>
        {
        private GloNetDbContext _context;

        public UserRepository(GloNetDbContext context)
            {
        ...
        public User Read(int key, bool noTracking = false, bool useNavigationProperties = false)
            ...
            try
                ...
                if(useNavigationProperties)
                    {
                    users = users.Include(u => u.ProfilePhoto).Include(u => u.FollowedUsers).Include(u => u.FollowersUsers).Include(u => u.Reactions).Include(u => u.Posts).Include(u => u.Comments);
                    }
